...
def transform_episodes_common_frame(episode_list, push_data):
    ...
    for tstep in range(num_steps):
        ...
        if ((tstep != 0) & (episode_idxs[prev_pose_idx] != episode_idxs[pose_idx])):
            ...
            obj_prevseq_last_pose = utils.vec3_to_pose2(push_data['obj_poses_2d'][prev_push_idx])
            obj_newseq_first_pose = utils.vec3_to_pose2(push_data['obj_poses_2d'][push_idx])
        
            prevseq_pose_set = True

        if prevseq_pose_set:
            obj_pose = utils.vec3_to_pose2(push_data['obj_poses_2d'][push_idx])            
            ee_pose = utils.vec3_to_pose2(push_data['ee_poses_2d'][push_idx])
            ...
            push_data_tf['obj_poses_2d'][push_idx] = utils.pose2_to_vec3(obj_pose_tf)
            push_data_tf['ee_poses_2d'][push_idx] = utils.pose2_to_vec3(ee_pose_tf)
